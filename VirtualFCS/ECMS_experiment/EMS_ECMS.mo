...
model EMS_ECMS
  ...
  String pyProgram = "
def ECMS_main(powerDemand, SoC, empty, b_V, sig, empty, fc_temp, pH2, pO2):
\t import numpy
\t import scipy
...
\t res = scipy.optimize.minimize(lambda x : ((((float(" + String(n_cell) + ") * (float(" + String(U_0) + ") - float(" + String(R) + ") * fc_temp / (2 * float(" + String(F) + ")) * math.log(1 / (pH2 / float(" + String(p_0) + ") * (pO2 / float(" + String(p_0) + "))**0.5)) - float(" + String(b_1_FC_stack) + ") * math.log10((x[0] + float(" + String(i_x_FC_stack) + ")) / float(" + String(i_0_FC_stack) + ")) + float(" + String(b_2_FC_stack) + ") * math.log10(1 - (x[0] + float(" + String(i_x_FC_stack) + ")) / float(" + String(I_max_FC_stack) + ")))) * x[0]) / (float(" + String(LHV_H2) + ") * (- 0.00000000013632059593 * x[0]**4 + 0.00000012503634527177 * x[0]**3 - 0.00003941319346342384 * x[0]**2 + 0.00426796522076351655 * x[0] + 0.43296967303427573486))) + (2.7 - 2*float(" + String(mu) + ")*(SoC - 0.5*(float(" + String(SOC_max) + ") + float(" + String(SOC_min) + ")))/(float(" + String(SOC_max) + ") - float(" + String(SOC_min) + "))) * ((float(sig) * float(b_V) * x[1]) / (float(" + String(LHV_H2) + ")))), numpy.array([0,0]), method='SLSQP', bounds=bounds, constraints=cons, options = {'maxiter':50})
...
equation
//---- Redefine variables ----
  SOC = sensorInputSOC * 0.01; // Convert from % to decimal number
  batt_V = batteryVoltage;
  powerD = powerRequest;
  OCV_batt = OCV;
  batt_I = batteryCurrent;
  eta_fc_sys_estimate = - 0.00000000013632059593 * set_current_3^4 + 0.00000012503634527177 * set_current_3^3 - 0.00003941319346342384 * set_current_3^2 + 0.00426796522076351655 * set_current_3 + 0.43296967303427573486;
//---- Recalculations ----
...
//---- Consumptions ----
  C_fc = (((n_cell*(U_0 - R*fcTemperature/(2*F)*log(1/(p_H2/p_0*(p_O2/p_0)^0.5)) - b_1_FC_stack*log10((set_current_1 + i_x_FC_stack)/i_0_FC_stack) + b_2_FC_stack*log10(1 - (set_current_1 + i_x_FC_stack)/I_max_FC_stack)))*set_current_1)/(LHV_H2*(- 0.00000000013632059593 * set_current_1^4 + 0.00000012503634527177 * set_current_1^3 - 0.00003941319346342384 * set_current_1^2 + 0.00426796522076351655 * set_current_1 + 0.43296967303427573486))); // Calculates C_fc the same as the optimization solver. This will vary from the actual FC since set_current_1 is used, and no dynamic behavious is considered.
...
// Battery is Discharging
    ...
    set_current_1 = Py.nineRealArgumentsReturnReal(pyHandle, powerD, SOC, 1, batt_V, sigma, 1, fcTemperature, p_H2, p_O2, pyProgram, pyModuleName, pyFunctionName); //Cost function is ran.
    C_sum = pre(C_sum) + C;
