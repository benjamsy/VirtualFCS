...
model FCSystem
  ...
  parameter Real m_FC_system(unit = "kg") = fuelCellStack.m_FC_stack + fCSubSystems.m_FC_subsystems;
...
//eta_FC_sys = max((Power_system)/(286000*(N_FC_stack*(fuelCellStack.pin_n.i + 0.000001)/(2*96485.3321))), 0.00001)*100;
  eta_FC_sys = max((Power_system)/(119959.2*(N_FC_stack* 2.016 * (fuelCellStack.pin_n.i + 0.000001)/(2*96485.3321))), 0.01)*100;
  ...
  connect(multiplex2.y, fCSubSystems.contolInput) annotation(
    Line(points = {{-42, -60}, {-32, -60}}, color = {0, 0, 127}, thickness = 0.5));
  connect(fCSubSystems.coolant_port_a, fuelCellStack.port_b_Coolant) annotation(
    Line(points = {{2, -36}, {4, -36}, {4, -6}}, color = {0, 127, 255}));
  connect(fCSubSystems.coolant_port_b, fuelCellStack.port_a_Coolant) annotation(
    Line(points = {{-2, -36}, {-6, -36}, {-6, -6}}, color = {0, 127, 255}));
  connect(fCSubSystems.air_port_a, fuelCellStack.port_b_Air) annotation(
    Line(points = {{18, -36}, {18, -2}}, color = {0, 127, 255}));
  connect(fCSubSystems.H2_port_a, fuelCellStack.port_b_H2) annotation(
    Line(points = {{-18, -36}, {-20, -36}, {-20, -2}}, color = {0, 127, 255}));
  connect(fCSubSystems.air_port_b, fuelCellStack.port_a_Air) annotation(
    Line(points = {{24, -36}, {40, -36}, {40, 22}, {18, 22}}, color = {0, 127, 255}));
  connect(fCSubSystems.H2_port_b, fuelCellStack.port_a_H2) annotation(
    Line(points = {{-24, -36}, {-40, -36}, {-40, 22}, {-20, 22}}, color = {0, 127, 255}));
