...
// OBS! TESTING WITH NEW COST FUNCTION
...
//---- Imports ----
  ...
  import OMCallPythonAdvanced.Py.twoRealArgumentsReturnReal;
  import OMCallPythonAdvanced.Py.fourRealArumentsReturnReal;
  import OMCallPythonAdvanced.Py.fiveRealArgumentsReturnReal;
  import OMCallPythonAdvanced.Py.sixRealArgumentsReturnReal;
  import OMCallPythonAdvanced.Py.nineRealArgumentsReturnReal;
  import Modelica.Utilities.Streams.print;
  //---- Parameters ----
  parameter Real ramp_up(unit = "1/s") = 30 "FC stack current ramp up rate" annotation(
  ...
  parameter Real I_max_FC_stack(unit = "A") "FC stack maximum current" annotation(
    Dialog(group = "Powertrain Parameters"));
  //Real i_L_FC_stack(unit = "A") = 800 "FC stack cell maximum limiting current";
  ...
  Real fc_V(unit = "V") "Fuel cell voltage real";
  ...
  Real batt_I(unit = "A") "Battery Current";
  ...
  final Real C_batt_2(start = 0);
  final Real C(start = 0);
  final Real C_sum(start = 0);
  Modelica.Blocks.Math.ContinuousMean C_a(u = C);
  final Real C_avg;
  ...
  String pyProgram = "
def ECMS_main(currentDemand, SoC, empty, b_V, sig, bopP, fc_temp, pH2, pO2):
\t import numpy
\t import scipy
...
\t return float(res.x[0])
  ...
  Modelica.Blocks.Interfaces.RealInput p_O2 annotation(
    Placement(visible = true, transformation(origin = {100, -120}, extent = {{-20, -20}, {20, 20}}, rotation = 90), iconTransformation(origin = {100, -120}, extent = {{-20, -20}, {20, 20}}, rotation = 90)));
  Modelica.Blocks.Continuous.Filter filter(analogFilter = Modelica.Blocks.Types.AnalogFilter.CriticalDamping, f_cut = 0.6, order = 5)  annotation(
...
//---- Redefine variables ----
  SOC = sensorInputSOC;
  fc_V = n_cell * (U_0 - R * fcTemperature / (2 * F) * log(1 / (p_H2 / p_0 * (p_O2 / p_0) ^ 0.5)) - b_1_FC_stack * log10((abs1.y + i_x_FC_stack) / i_0_FC_stack) + b_2_FC_stack * log10(1 - (abs1.y + i_x_FC_stack) / I_max_FC_stack));
...
//Polynomial curve fitting if internalResistance1 is used.
    eta_b_dischr = 0.5 * (1 + sqrt(1 - 4 * internalResistance1 * batt_I * batt_V / OCV_batt ^ 2));
    eta_b_chr = 1;
...
//Polynomial curve fitting if internalResistance1 is used.
    eta_b_chr = 2 / (1 + sqrt(1 - 4 * internalResistance1 * batt_I * batt_V / OCV_batt ^ 2));
    eta_b_dischr = 1;
  else
...
// Battery is Discharging
    ...
    set_current_1 = Py.nineRealArgumentsReturnReal(pyHandle, currentD, SOC, 1, batt_V, sigma, bop_power, fcTemperature, p_H2, p_O2, pyProgram, pyModuleName, pyFunctionName);
//Cost function is ran.
    C_sum = pre(C_sum) + C;
