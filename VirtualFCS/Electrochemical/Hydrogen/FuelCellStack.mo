...
potentialSource.v =N_FC_stack*(1.229 - (R*temperatureSensor.T)/(2*F)*log(p_H2*(p_O2^0.5)) - (0.85*0.001)*(temperatureSensor.T - 298.15) - R_O_FC_stack*(abs(currentSensor.i)/A_FC_surf) - (R*temperatureSensor.T)/(2*F*0.3419)*log(abs(min(currentSensor.i, -0.00001)/A_FC_surf)/i_0_FC_stack) + (R*1.4672*temperatureSensor.T)/(2*F)*log(1-(abs(currentSensor.i/A_FC_surf)/i_L_FC_stack)));
...
//*** DEFINE CONNECTIONS ***//
 ...
 connect(O2_sink.T_in, temperatureSensor.T) annotation(
    Line(points = {{72, 44}, {-50, 44}, {-50, -80}}, color = {0, 0, 127}));
  annotation(
