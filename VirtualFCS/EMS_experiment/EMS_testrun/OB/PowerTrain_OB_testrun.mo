within VirtualFCS.EMS_experiment.EMS_testrun.OB;

model PowerTrain_OB_testrun
  // System
  ...
  VirtualFCS.EMS_experiment.EMS_ECMS ems_ecms(I_max_FC_stack = 130, I_max_batt = 360, I_min_FC_stack = 20, I_min_batt = -180, SOC_max = 0.8, SOC_min = 0.25, n_cell = N_FC_stack, t_const = 0)  annotation(
    Placement(visible = true, transformation(origin = {-60, -30}, extent = {{-20, -10}, {20, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression getpH2(y = fuelCellSystem_testrun.fuelCellStack.teeJunctionVolume1.port_3.p*100000)  annotation(
    Placement(visible = true, transformation(origin = {-110, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression getFCTemp(y = fuelCellSystem_testrun.fuelCellStack.temperatureSensor.T)  annotation(
  ...
  Modelica.Blocks.Sources.RealExpression getpO2(y = fuelCellSystem_testrun.fuelCellStack.teeJunctionVolume.port_3.p * 100000)  annotation(
...
equation
  C_fc_real = (Power_FC)/(ems_ecms.LHV_H2*(fuelCellSystem_testrun.eta_FC_sys*0.01));
  ...
  if Power_del_DC_DC > 0 then
    eta_drivetrain = min(max(((Power_del_DC_DC)/max((Power_FC + Power_batt + ((Power_FC*(1 - (fuelCellSystem_testrun.eta_FC_sys*0.01))) + (Power_batt*(1 - (batterySystem.eta_batt*0.01))) + (Power_del_DC_DC*(1 - (eta_DC_DC*0.01))))), 0.000001))*100, 0), 100);
  else
    eta_drivetrain = min(max(((-Power_del_DC_DC)/max((Power_FC - Power_batt + ((Power_FC*(1 - (fuelCellSystem_testrun.eta_FC_sys*0.01))) - (Power_batt*(1 - (batterySystem.eta_batt*0.01))) - (Power_del_DC_DC*(1 - (eta_DC_DC*0.01))))), 0.000001))*100, 0), 100);
  end if;
  ...
  connect(getpO2.y, ems_ecms.p_O2) annotation(
    Line(points = {{-98, -100}, {-50, -100}, {-50, -42}}, color = {0, 0, 127}));
  connect(getBattCurrent.y, gain.u) annotation(
    Line(points = {{-98, -80}, {-84, -80}}, color = {0, 0, 127}));
  connect(gain.y, ems_ecms.batteryCurrent) annotation(
    Line(points = {{-70, -80}, {-60, -80}, {-60, -42}}, color = {0, 0, 127}));
  connect(getBattVoltage.y, ems_ecms.batteryVoltage) annotation(
    Line(points = {{-98, -60}, {-70, -60}, {-70, -42}}, color = {0, 0, 127}));
  ...
  connect(getPowerRequest.y, ems_ecms.powerRequest) annotation(
    Line(points = {{-98, 0}, {-70, 0}, {-70, -18}}, color = {0, 0, 127}));
  connect(getFCTemp.y, ems_ecms.fcTemperature) annotation(
    Line(points = {{-98, 20}, {-60, 20}, {-60, -18}}, color = {0, 0, 127}));
  connect(getpH2.y, ems_ecms.p_H2) annotation(
    Line(points = {{-98, 40}, {-50, 40}, {-50, -18}}, color = {0, 0, 127}));
  connect(ems_ecms.controlOutputFuelCellCurrent, product.u2) annotation(
    Line(points = {{-38, -30}, {-26, -30}, {-26, 2}, {54, 2}, {54, -12}, {48, -12}}, color = {0, 0, 127}));
protected
  annotation(
