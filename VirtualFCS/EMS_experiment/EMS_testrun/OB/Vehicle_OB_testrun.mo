...
model Vehicle_OB_testrun
  ...
  VirtualFCS.EMS_experiment.EMS_testrun.OB.PowerTrain_OB_testrun powerTrain_OB_testrun(C_bat_pack = 180, SOC_init = 0.3, V_max_bat_pack = 54.75, V_min_bat_pack = 37.5, V_nom_bat_pack = 48) annotation(
...
//end when;
  if driveCycle_EMS_experiment.P > 0 then
    eta_vehicle = max(((driveCycle_EMS_experiment.P)/max((driveCycle_EMS_experiment.P + (powerTrain_OB_testrun.Power_del_DC_DC*(1 - (powerTrain_OB_testrun.eta_drivetrain*0.01))) + (driveCycle_EMS_experiment.P*(1 - driveCycle_EMS_experiment.eff_drivetrain))), 0.00001))*100, 0);
  else
    eta_vehicle = max(((abs(driveCycle_EMS_experiment.P))/max(abs(driveCycle_EMS_experiment.P) + (abs(powerTrain_OB_testrun.Power_del_DC_DC)*(1 - (powerTrain_OB_testrun.eta_drivetrain*0.01))) + (abs(driveCycle_EMS_experiment.P)*(1 - driveCycle_EMS_experiment.eff_brake)), 0.00001))*100, 0);
  end if;
  connect(driveCycle_EMS_experiment.pin_p, powerTrain_OB_testrun.pin_p) annotation(
    Line(points = {{-22, 10}, {22, 10}}, color = {0, 0, 255}));
  connect(driveCycle_EMS_experiment.pin_n, powerTrain_OB_testrun.pin_n) annotation(
    Line(points = {{-22, -10}, {22, -10}}, color = {0, 0, 255}));
protected
  annotation(
    experiment(StartTime = 0, StopTime = 1490, Tolerance = 0.0001, Interval = 1));
end Vehicle_OB_testrun;
