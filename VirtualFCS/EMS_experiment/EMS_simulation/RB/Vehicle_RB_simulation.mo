...
equation
  ...
  if driveCycle_EMS_experiment.P > 0 then
    ...
    eta_vehicle = max(((abs(driveCycle_EMS_experiment.P))/max(abs(driveCycle_EMS_experiment.P) + (abs(powerTrain_RB_simulation.Power_del_DC_DC)*(1 - (powerTrain_RB_simulation.eta_drivetrain*0.01))) + (abs(driveCycle_EMS_experiment.P)*(1 - driveCycle_EMS_experiment.eff_brake)), 0.00001))*100, 0);
  end if;
  connect(driveCycle_EMS_experiment.pin_p, powerTrain_RB_simulation.pin_p) annotation(
    Line(points = {{-22, 10}, {22, 10}}, color = {0, 0, 255}));
  connect(driveCycle_EMS_experiment.pin_n, powerTrain_RB_simulation.pin_n) annotation(
    Line(points = {{-22, -10}, {22, -10}}, color = {0, 0, 255}));
protected
  annotation(
    experiment(StartTime = 0, StopTime = 1490, Tolerance = 0.0001, Interval = 1));
end Vehicle_RB_simulation;
